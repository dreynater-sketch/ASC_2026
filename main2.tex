...
\TODO{Sputtering recipe paragraph. CONFIRMED: poster numbers are correct -- Ta at 100 W / $-$100 V bias / 10 mTorr Ar, Nb at 700 nm. (results\_2piece\_tanb.tex's 60 W / 750 nm was the old recipe; this work was redone for the poster, so the poster is authoritative throughout -- same resolution applies to the simulated Cu ceiling below.) Just needs writing up in your own words: 500 nm Ta diffusion barrier by DC magnetron sputtering at 100 W with $-$100 V substrate bias in 10 mTorr Ar at \SI{200}{\celsius}, followed by 700 nm Nb at 250 W in 8 mTorr Ar, also at \SI{200}{\celsius}. Nb thickness exceeds three RF penetration depths at 11 GHz, so the measured surface impedance reflects the film rather than the copper beneath it.}
...
Combined with a literature bulk-copper conductivity, the same simulation gives a simulated bare-copper ceiling of $Q_0=77{,}000$ ($R_s=5.00\,\mathrm{m}\Omega$), used in Section~V as a cross-check on the measured copper baseline. \TODO{CONFIRMED: 77,000 is the correct, current simulated value for this cavity -- the 76,000 in thesis Eq.~8.3 was from the earlier Nb3Sn-era work, not applicable here. Just needs a sentence in your own words.}
...
\TODO{CORRECTION: I originally mislabeled this paragraph as "adapted from thesis diffusion-barrier discussion" -- that was wrong, checked against the actual thesis text and there is no such passage there. This is my own drafted text from earlier in this session, not ported from anything of yours. Treat it as needing a full voice pass, same as the other TODOs, not as already-your-words. Facts to keep: $R_s$ rises gradually then increases sharply near 9.2 K (bulk-Nb $T_c$); above $T_c$, $Q_0$ falls to order $10^4$, below the bare-Cu baseline, because normal-state Nb is more resistive than annealed Cu and the film exceeds its normal-state skin depth at 11 GHz; the transition sitting at the bulk-Nb $T_c$ (not a suppressed value) confirms the Ta layer is an effective diffusion barrier and the film is close to stoichiometric bulk Nb. Your thesis DOES have a related but different passage -- the barrier-comparison table for Ta/Nb/W blocking Cu interdiffusion into Nb3Sn -- which could inform phrasing here, but the argument itself (clean $T_c$ transition = effective barrier) is one I constructed for this specific film, not lifted from that table.}
