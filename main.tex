...
Superconducting films coated on 3D cavities can raise the quality factor of a copper haloscope or accelerator cavity by orders of magnitude. There are many different superconducting materials, and many different ways to make them, so a robust and quick method for testing the quality factor of the superconducting film is required. There are currently many ways to test the RF properties of superconducting films and here we report the fabrication and 11\,GHz RF characterization of a two-piece hexagonal oxygen-free high-conductivity (OFHC) copper cavity sputter-coated with a \SI{500}{\nano\meter} Ta diffusion barrier and a \SI{700}{\nano\meter} Nb film. The cavity was measured from 2 to \SI{10}{\kelvin} in a Quantum Design Physical Property Measurement System (PPMS), and the unloaded quality factor $Q_0$ was extracted from vector network analyzer $S_{11}$, $S_{22}$, and $S_{21}$ data using a diameter-correction circle-fit for the input and output coupling coefficients. For comparison, the bulk Cu cavity was simulated giving $Q_0=77{,}000$ ($R_s=\SI{5.00}{\milli\ohm}$), and the measured Cu cavity gave $Q_0=62{,}000$ ($R_s=\SI{5.65}{\milli\ohm}$), only 19\% below the simulated ideal. The Ta/Nb-coated cavity reached $Q_0 = 1.64\times10^{6}$ at \SI{2}{\kelvin}, corresponding to a surface resistance $R_s = \SI{213}{\micro\ohm}$ for a simulated geometric factor $G=\SI{350}{\ohm}$, roughly 250 times the BCS-limited surface resistance of niobium at this frequency, indicating that residual resistance dominated. This cavity design allows for pre- and post-deposition surface analysis to understand what can be improved in future generations for fast optimization throughput.
\end{abstract}
...
\begin{figure}[!t]
    \centering
    \includegraphics[width=0.9\linewidth]{cavity_photo.png}
    \caption{(a) The two-piece hexagonal Cu/Ta/Nb cavity after sputter deposition of the Ta diffusion barrier and Nb film. (b) Micrograph of the coated wall near the seam, showing bare Cu still visible after deposition.}
    \label{fig:cavity}
\end{figure}
...
The measured bare-copper baseline ($Q_0=62{,}000$, $R_s=\SI{5.65}{\milli\ohm}$) is within 19\% of the independently simulated bulk-Cu ceiling ($Q_0=77{,}000$), confirming that the cavity geometry, fabrication, and measurement method reproduce the expected bulk-Cu performance. A separate finding on the same bare-copper cavity points to an easy path to a higher baseline still: sealing its seam with indium foil raised $Q_0$ from $55,000$ to $62,000$, a simple treatment we plan to apply to the coated cavity in future work. Against this validated baseline, the Ta/Nb-coated film reaches $Q_0=1.64\times10^{6}$ and $R_s=\SI{213}{\micro\ohm}$ at \SI{2}{\kelvin}, an improvement of roughly $26\times$. Against the BCS-limited value for niobium at this frequency ($R_s \approx \SI{0.58}{\micro\ohm}$ at \SI{2}{\kelvin}, $Q_0\approx 6.0\times10^{8}$), however, the measured film sits roughly 250 times higher in surface resistance. Uncoated areas of the substrate are a likely dominant mechanism: Fig.~\ref{fig:cavity}(b) shows bare Cu still visible near the coated wall after deposition, consistent with incomplete film coverage.
...
These results add to the growing pool of literature hoping to characterize the RF properties superconducting films. We aimed to establish a benchmark with Nb. More work should be done to improve substrate coating coverage and the seam gap between the cavities before moving to more complex, higher-$H_{c2}$ coating such as Nb$_3$Sn.

\section{Conclusion}

A two-piece hexagonal OFHC copper cavity was machined, characterized bare, and then coated with a sputtered Ta/Nb bilayer and characterized again at \SI{11}{\giga\hertz} from 2 to \SI{10}{\kelvin}. The bare-copper baseline matched an independently simulated bulk-Cu ceiling to within 19\%, validating the cavity design and measurement method. Using a diameter-correction circle-fit to extract coupling coefficients from $S_{11}$ and $S_{22}$, the unloaded quality factor of the Ta/Nb-coated cavity at \SI{2}{\kelvin} was found to be $Q_0=1.64\times10^{6}$ ($R_s=\SI{213}{\micro\ohm}$), roughly $26\times$ the same body's bare-copper baseline, but approximately 250 times the BCS-limited surface resistance of niobium at this frequency, an excess more consistent with incomplete film coverage than with trapped-flux resistance. Demonstrating this within the \SI{26}{\milli\meter} sample space of a commercial PPMS, rather than a purpose-built haloscope magnet with six times the bore, shows that coatings can be screened without dedicated large-bore infrastructure. This measurement establishes a baseline against which future work on the same platform can be compared with. This design allows for surface analysis of the sputtered film, and full quality factor characterization down to 2K and in high magnetic field with an instrument commonly found in many labs. 
