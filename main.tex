...
\usepackage{cite}
\usepackage{xcolor}
\newcommand{\claude}[1]{\textcolor{blue}{#1}}
\graphicspath{{images/}}
...
At \SI{2}{\kelvin}, the film gives $Q_0=1.64\times10^{6}$ and $R_s=\SI{213}{\micro\ohm}$. Against the measured bare-copper baseline ($Q_0=62{,}000$, $R_s=\SI{5.65}{\milli\ohm}$), this is an improvement of roughly $26\times$ in quality factor, and it is comparable with the independently simulated bulk-Cu ceiling of $Q_0=77{,}000$. A separate finding on the same bare-copper cavity points to an easy path to a higher baseline still: sealing its seam with indium foil raised $Q_0$ from $55,000$ to $62,000$, a simple treatment we plan to apply to the coated cavity in future work. Against the BCS-limited value for niobium at this frequency ($R_s \approx \SI{0.58}{\micro\ohm}$ at \SI{2}{\kelvin}, $Q_0\approx 6.0\times10^{8}$), however, the measured film sits roughly 250 times higher in surface resistance. \claude{This gap is too large to explain with the classical roughness-enhanced-field mechanism, which only becomes significant at high RF magnetic field; at the low fields used here, a more basic coating-coverage problem is the likely cause.} Fig.~\ref{fig:roughness} \claude{shows why: the substrate carries scratches up to \SI{11}{\micro\meter} deep, more than an order of magnitude deeper than the \SI{700}{\nano\meter} Nb film. A film this thin cannot conformally coat a defect that deep, and the resulting coating artifact --- thinned or discontinuous Nb along the scratch walls --- is a more direct loss mechanism than local field enhancement from ordinary roughness.} \claude{A Veeco laser-scanning confocal microscope scan of the sputtered Nb surface shows} a background roughness of roughly $\pm$\SI{2}{\micro\meter} punctuated by scattered grooves reaching \SIrange{-8}{-11}{\micro\meter} along the line scan marked in red\claude{, consistent with this coating-artifact picture rather than with a subtle field-enhancement effect}. For comparison, electropolished Nb-on-Cu films in the literature reach AFM roughness (Ra) of only \SIrange{6}{21}{\nano\meter}~\cite{Pira2019CuPolish}\claude{, with no comparably deep defects for a thin film to fail to cover}.
