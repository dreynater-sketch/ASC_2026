...
\begin{figure*}[!t]
    \centering
    \includegraphics[width=.5\textwidth]{sim_modes.png}
    \caption{Simulated cavity modes showing electric field volume for a) TM$_{010}$ mode and the two nearest higher-order modes, b) TM$_{011}$ and c) TM$_{012}$. This simulations was used to verify the frequency and geometric factor of the mode, as well as calculate the ideal quality factor of the Cu cavity. The color here is used to show how the mode structure looks with electric field being highest with white and lowest with black and is not to be analyzed quantitatively.}
    \label{fig:sim}
\end{figure*}
...
At \SI{2}{\kelvin}, the film gives $Q_0=1.64\times10^{6}$ and $R_s=\SI{213}{\micro\ohm}$. Against the measured bare-copper baseline ($Q_0=62{,}000$, $R_s=\SI{5.65}{\milli\ohm}$), this is an improvement of roughly $26\times$ in quality factor, and it is comparable with the independently simulated bulk-Cu ceiling of $Q_0=77{,}000$. Sealing the cavity seam with indium foil raised $Q_0$ from $55,000$ to $62,000$, a simple step we plan to apply to the coated cavity in future work. Comparing the measured Nb film to the ideal niobium at this frequency ($R_s \approx \SI{0.58}{\micro\ohm}$ at \SI{2}{\kelvin}, $Q_0\approx 6.0\times10^{8}$), which therefore means our nb film cavity sits at roughly 250 times higher in surface resistance. Surface roughness and uncoated substrate areas are the dominant mechanisms that reduced our quality factor. Fig.~\ref{fig:roughness} confirms this directly: a Veeco laser-scanning confocal microscope scan of the sputtered Nb surface shows a background roughness of roughly $\pm$\SI{2}{\micro\meter} punctuated by scattered grooves reaching \SIrange{-8}{-11}{\micro\meter} along the line scan marked in red, consistent with polishing scratches that locally disrupted superconductivity. For comparison, electropolished Nb-on-Cu films in the literature reach AFM roughness (Ra) of only \SIrange{6}{21}{\nano\meter}~\cite{Pira2019CuPolish}, two to three orders of magnitude below what is measured here.
